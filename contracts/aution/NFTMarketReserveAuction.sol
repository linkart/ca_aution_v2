  ...
  NFTMarketAuction
{
  ...
  mapping(address => mapping(uint256 => uint256)) private nftTokenIdToAuctionId;
   ...
   */
  function getReserveAuctionIdFor(address nft, uint256 tokenId) public view returns (uint256) {
    return nftTokenIdToAuctionId[nft][tokenId];
  }
   ...
   */
  function getSellerFor(address nft, uint256 tokenId)
    internal
    view
    virtual
    returns (address)
  {
    address seller = auctionIdToAuction[nftTokenIdToAuctionId[nft][tokenId]].seller;
    if (seller == address(0)) {
  ...
  function createReserveAuction(
    ...
    nftTokenIdToAuctionId[nft][tokenId] = auctionId;
  ...
  function cancelReserveAuction(uint256 auctionId) public nonReentrant {
    ...
    delete nftTokenIdToAuctionId[auction.nft][auction.tokenId];
  ...
  function finalizeReserveAuction(uint256 auctionId) public nonReentrant {
    ...
    IERC721(auction.nft).transferFrom(address(this), auction.bidder, auction.tokenId);

    _distributeFunds(auction.seller, auction.shareUser, auction.amount);

    emit ReserveAuctionFinalized(auctionId, auction.seller, auction.bidder, auction.nft, auction.tokenId, auction.amount);
  }
